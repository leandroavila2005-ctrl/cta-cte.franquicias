...
//  El token se manda al backend en cada request; acá solo se decodifica para
//  mostrar el email (no es fuente de verdad: la valida el Apps Script).
//  Solo se usa en modo real (DEMO_MODE:false).
// ============================================================================
window.Genova = window.Genova || {}

Genova.auth = (function () {
  ...
  function init(onCredential) {
    ...
    var cid = Genova.config.OAUTH_CLIENT_ID
    if (!cid || cid.indexOf('PEGAR') === 0) throw new Error('Falta configurar OAUTH_CLIENT_ID en js/config.js')
  ...
  // Dispara One Tap y, como respaldo visible, renderiza el botón oficial de Google.
